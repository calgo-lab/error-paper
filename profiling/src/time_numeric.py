...
def time_one_error_model_numeric(desc_string: str, e_mech: error_mechanism, e_type: error_type, e_rate: float = 0.5, max_n: int = 1000000, runs: int = 100):  # noqa: E501, PLR2044
    """
    ...
    while n_row <= max_n:  # will halt if max_n isn't dumb
        n_col = 2
        while n_col <= 10:
...
def time_tab_err_numeric(max_n: int = 1000000, runs: int = 100, seed: int | None = 42, write_path: str = "../results/numeric_times_new.csv") -> pd.DataFrame:  # noqa: E501
    ...
    e_type_dict = {"AddDelta": error_type.AddDelta(),
                   "Outlier": error_type.Outlier(),
                   "MissingValue": error_type.MissingValue(),
                   "MistypeInt": error_type.Mistype({"mistype_dtype": "int64"}),
                   "WrongUnitScale10": error_type.WrongUnit({"wrong_unit_scaling": lambda x: x * 10})}
    ...
    for e_type_name, e_type in e_type_dict.items():
        for e_rate in e_rate_list:
            ...
            dataframes.append(time_one_error_model_numeric(desc, e_mech, e_type, e_rate, max_n, runs))
