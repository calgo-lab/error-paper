...
def run_error_mechanism_experiment(data: pd.DataFrame, error_rate: float, machine_learning_model: BaseEstimator, evaluation_function: Callable, seed: int, folds: int = 5) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Run ML models
    clean_acc, dirty_acc, dirty_error_prop = evaluate_on_dirty_data(data, error_rate=error_rate, evaluation_function=evaluation_function, model=machine_learning_model, n_splits=folds, error_types_to_include=[error_type.WrongUnit(), error_type.Erroneous()], seed=seed)
    clean_acc, no_ear_enar_acc, no_ear_enar_error_prop = evaluate_on_dirty_data(data, error_rate=error_rate, evaluation_function=evaluation_function, model=machine_learning_model, n_splits=folds, error_types_to_include=[error_type.WrongUnit(), error_type.Erroneous()], error_mechanisms_to_exclude=[error_mechanism.EAR(), error_mechanism.ENAR()], seed=seed)
    clean_acc, no_enar_ecar_acc, no_enar_ecar_error_prop = evaluate_on_dirty_data(data, error_rate=error_rate, evaluation_function=evaluation_function, model=machine_learning_model, n_splits=folds, error_types_to_include=[error_type.WrongUnit(), error_type.Erroneous()], error_mechanisms_to_exclude=[error_mechanism.ENAR(), error_mechanism.ECAR()], seed=seed)
    clean_acc, no_ear_ecar_acc, no_ear_ecar_error_prop = evaluate_on_dirty_data(data, error_rate=error_rate, evaluation_function=evaluation_function, model=machine_learning_model, n_splits=folds, error_types_to_include=[error_type.WrongUnit(), error_type.Erroneous()], error_mechanisms_to_exclude=[error_mechanism.EAR(), error_mechanism.ECAR()], seed=seed)
...
def main():
    ...
    seed = 1234
    experiment_name = "classification-wrongunit-typo"
    ...
    for dataset_id in ids:
        for error_rate in [0.1, 0.25, 0.5, 0.75, 0.9]:
            ...
            # Model setup - This needs to change between regression and classification
