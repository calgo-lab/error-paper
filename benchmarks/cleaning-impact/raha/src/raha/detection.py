...
class Detection:
    ...
    def initialize_dataset(self, d):
        ...
        # d.dictionary = dd
        d.results_folder = os.path.join('./', "raha-baran-results-" + d.name)
        ...
        if not self.STRATEGY_FILTERING:
            if os.path.exists(sp_folder_path):
                ...
                with ProcessPoolExecutor() as executor:
                    strategy_profiles_list = list(executor.map(self._strategy_runner_process, algorithm_and_configurations))
        else:
    ...
    def label_with_ground_truth(self, d):
        ...
        actual_errors_dictionary = d._get_actual_errors_dictionary_ground_truth()
        for j in range(d.dataframe.shape[1]):
...
if __name__ == "__main__":
    ...
    dataset = Dataset(dataset_name, scenario, version)

    app = Detection()
    app.VERBOSE = True
    app.SAVE_RESULTS = False
    detected_cells = app.run(dataset)
    p, r, f1 = dataset.get_data_cleaning_evaluation(detected_cells)[:3]
    print("Raha's performance on {}:\nPrecision = {:.2f}\nRecall = {:.2f}\nF1 = {:.2f}".format(dataset.name, p, r, f1))
    detected_cells_index = list(detected_cells.keys())
    with open(dataset.raha_result_path, "wt") as f:
        results = {
        ...
        json.dump(results, f)
    print("Wrote detected cells to disk.")
