...
class Correction:
    ...
    def __init__(self):
        ...
        self.LABELING_ERROR_PCT = 0  # simulates pct with which user provides wrong labels
    ...
    def label_with_ground_truth(self, d):
        ...
        for j in range(d.dataframe.shape[1]):
            ...
            # Defective user input simulation: with a cance of LABELING_ERROR_PCT,
            # another randomly chosen value of a column replaces the ground truth value.
            if random.random() < self.LABELING_ERROR_PCT:
                user_error_candidates = d.clean_dataframe.loc[d.clean_dataframe.iloc[:, j] != ground_truth_value].iloc[:, j]
...
if __name__ == "__main__":
    ...
    scenario = "scenario"
    version = "0"
    error_detection_mode = 'perfect'  # 'raha' to load raha-detection results

    data = Dataset(dataset_name, scenario, version)
    app = Correction()
    app.VERBOSE = True
