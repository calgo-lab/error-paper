...
def run_error_mechanism_experiment(data: pd.DataFrame, error_rate: float, machine_learning_model: BaseEstimator, evaluation_function: Callable, seed: int, folds: int = 5) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Run ML models
    clean_acc, dirty_acc, dirty_error_prop = evaluate_on_dirty_data(data, error_rate=error_rate, evaluation_function=evaluation_function, model=machine_learning_model, n_splits=folds, error_types_to_include=[error_type.Outlier(), error_type.Typo()], seed=seed)
    clean_acc, no_ear_enar_acc, no_ear_enar_error_prop = evaluate_on_dirty_data(data, error_rate=error_rate, evaluation_function=evaluation_function, model=machine_learning_model, n_splits=folds, error_types_to_include=[error_type.Outlier(), error_type.Typo()], error_mechanisms_to_exclude=[error_mechanism.EAR(), error_mechanism.ENAR()], seed=seed)
...
def main():
    ...
    seed = 1234
    experiment_name = "classification-outlier-typo"
